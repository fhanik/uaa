...
/**
 * Test context for tests that require zone-namespaced session behavior (SessionZoneResolutionFilter
 * and Spring Session filter in the MockMvc chain). Use only for tests that verify per-zone session
 * isolation; most tests should use {@link DefaultTestContext}.
 */
...
@SpringBootTest(
        ...
        classes = {
                ...
                ZonePathSessionTestContext.ZonePathSessionMockMvcTestConfig.class,
...
public @interface ZonePathSessionTestContext {
    ...
    class ZonePathSessionMockMvcTestConfig {
        @Bean

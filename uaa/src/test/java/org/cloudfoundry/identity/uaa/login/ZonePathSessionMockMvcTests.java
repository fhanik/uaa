...
import org.cloudfoundry.identity.uaa.ZonePathSessionTestContext;
...
import org.cloudfoundry.identity.uaa.session.ZoneNamespacedSessionRepository;
 ...
 * Verify zone-namespaced session behavior when using path-based zones (/z/{subdomain}/): one JSESSIONID
 * can hold separate sessions per zone, and logging out of one zone does not affect others.
 * <p>
 * Uses {@link ZonePathSessionTestContext} so that {@code servlet.session-store=memory} and
 * {@link ZoneNamespacedSessionRepository} are active; {@link DefaultTestContext} adds the session
 * filters to the MockMvc chain when present. The browse-back test
 * ({@link #loginToDefaultZoneAndBothPathZonesThenVerifyProfileInEach}) is skipped when the
 * context does not provide ZoneNamespacedSessionRepository (e.g. due to context reuse).
 */
@ZonePathSessionTestContext
@TestPropertySource(properties = "test.zone.session.mockmvc=true")
class ZonePathSessionMockMvcTests {
     ...
     * With zone-namespaced sessions (servlet.session-store=memory), DefaultTestContext adds
     * SessionZoneResolutionFilter and the session repository filter to the MockMvc chain,
     * so one JSESSIONID holds separate sessions per zone. Verifies browse-back: after logging in
     * to all three zones, visiting each zone's profile again shows the correct user.
     */
    @Test
    void loginToDefaultZoneAndBothPathZonesThenVerifyProfileInEach() throws Exception {
        Assumptions.assumeTrue(sessionRepository instanceof ZoneNamespacedSessionRepository,
                "Requires ZonePathSessionTestContext (servlet.session-store=memory) and ZoneNamespacedSessionRepository; got " + (sessionRepository != null ? sessionRepository.getClass().getName() : "null"));
        ...
        // Browse back: each zone still shows the correct user (per-zone sessions)
        mockMvc.perform(withSessionCookie(MODE.createRequestBuilder("", HttpMethod.GET, "/profile"), session))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Account Settings")))
                .andExpect(content().string(containsString(DEFAULT_ZONE_USER)));
